...
app.put('/feeditem/:feeditemid/comments/:commentindex/likelist/:userid', function(req, res) {
  ...
  if(fromUser === userId) {
    db.collection('feedItems').findOne({ _id: feedItemId}, function(err, feedItemData) {
      ...
      db.collection('feedItems').updateOne({ _id: feedItemId}, {$set: {comments: feedItemData.comments}}, function(err) {
        if(err) {
          return sendDatabaseError(res, err);
        }
        var userList = [comment.author];
        resolveUserObjects(userList.concat(comment.likeCounter), function(err, map) {
          if(err) {
            return sendDatabaseError(res, err);
          }
          comment.author = map[comment.author];
          comment.likeCounter.map((userId) => map[userId])
          res.send(comment);
        });
      });
    });
