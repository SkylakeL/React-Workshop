...
app.delete('/feeditem/:feeditemid', function(req, res) {
  ...
  if (feedItem.contents.author === fromUser) {
    require('./database').deleteDocument('feedItems', feedItemId);
    // Remove references to this feed item from all other feeds.
    var feeds = require('./database').getCollection('feeds');
    ...
    feedIds.forEach((feedId) => {
      ...
      if (itemIdx !== -1) {
        ...
        // Update feed.
        require('./database').writeDocument('feeds', feed);
      }
    });
...
validate({body: CommentSchema}), function(req, res) {
  ...
  if (fromUser === body.author) {
    var newUpdate = commentsUpdate(body.feedItemId, body.author, body.contents);
      // When POST creates a new resource, we should tell the client about it
      // in the 'Location' header and use status code 201.
      res.status(201);
      res.set('Location', '/comment/' + body.feedItemId);
      newUpdate.comments.forEach((comment) => {
        comment.author = readDocument('users', comment.author)
      });
      res.send(newUpdate);
    } else {
...
function commentsUpdate(feedItemId, author, contents) {
  var time = new Date().getTime();
  var feedItem = readDocument('feedItems', feedItemId);
  ...
  app.put('/likeComment', function(req, res) {
    var fromUser = getUserIdFromToken(req.get('Authorization'));
    var body = req.body;
    var feedItemId = body.feedItemId;
    var userId = body.userId;
    var commentIdx = body.commentIdx;
    if (fromUser === userId) {
      var feedItem = readDocument('feedItems', feedItemId);
      ...
      if (comment.likeCounter.indexOf(userId) === -1) {
        ...
        feedItem.comments[commentIdx] = comment
        writeDocument('feedItems', feedItem);
      }
      // Return a resolved version of the likeCounter
      var resComment = feedItem.comments[commentIdx];
      resComment.author = readDocument('users', comment.author);
      res.send(resComment);
    } else {
      // 401: Unauthorized.
      res.status(401).end();
...
app.delete('/unlikeComment', function(req, res) {
  var fromUser = getUserIdFromToken(req.get('Authorization'));
  var body = req.body;
  var feedItemId = body.feedItemId;
  var userId = body.userId;
